...
async def setup_machine(machine_id, mkosi_kernel_dir):
    proc = await asyncio.subprocess.create_subprocess_exec(
        "mkosi",
        "--machine",
        machine_id,
...
async def machine_pool(num_machines, mkosi_kernel_dir):
    ...
    procs = await asyncio.gather(
        *(
            setup_machine(machine_id, mkosi_kernel_dir)
...
async def test(
    test, machine_pool, mkosi_kernel_dir, fstests_dir, record_property
):
    ...
    async with get_machine(machine_pool) as machine_id:
        ...
        record_property("stdout", stdout.decode())
        record_property("stderr", stderr.decode())
